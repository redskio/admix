...
export default function CreativeEditPage({ params }: PageProps) {
  const router = useRouter()
  const creative = mockCreatives.find((c) => c.id === params.id)
  const creativeIndex = mockCreatives.findIndex((c) => c.id === params.id)
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <Badge
          className={`ml-auto shrink-0 text-xs px-2.5 py-1 border font-medium ${status.className}`}
          variant="outline"
        >
          {status.label}
        </Badge>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Card className="border-gray-200">
            ...
            <CardContent className="space-y-5">
              ...
              {/* 후킹 문구 */}
              <div className="space-y-2">
                <Label htmlFor="hookCopy" className="text-sm font-medium text-gray-700">
                  후킹 문구
                </Label>
                <textarea
                  id="hookCopy"
                  ...
                  placeholder="광고 후킹 문구를 입력하세요"
                  rows={4}
                  className="w-full text-sm border border-input rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none placeholder:text-muted-foreground bg-background"
                />
          ...
          <Card className="border-gray-200 overflow-hidden">
            <div className={`h-48 bg-gradient-to-br ${gradient} flex flex-col items-center justify-center relative`}>
              ...
              <div className="absolute bottom-3 right-3">
                <Badge className={`text-xs border ${status.className}`} variant="outline">
                  {status.label}
                </Badge>
              </div>
            </div>
              ...
              {hookCopy && (
                <>
                  ...
                  <div className={`rounded-lg bg-gradient-to-br ${gradient} p-3`}>
                    <p className="text-white text-xs italic font-medium text-center leading-relaxed drop-shadow">
                      &ldquo;{hookCopy}&rdquo;
                    </p>
                  </div>
                </>
              )}
